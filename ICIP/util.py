...
class DatasetDirectory(Dataset):
    ...
    def __getitem__(self, index):
        ...
        data = torch.from_numpy(np.transpose(np.array(reader)).astype('float32'))

        f = h5py.File(os.path.join(self.omega_path, self.mat_list[index]), 'r')
        reader = f.get('omega')
        omega = torch.from_numpy(np.transpose(np.array(reader)).astype('float32'))

        f = h5py.File(os.path.join(self.gt_path, self.mat_list[index]), 'r')
        reader = f.get('radiance')
        target = torch.from_numpy(np.transpose(np.array(reader)).astype('float32'))
