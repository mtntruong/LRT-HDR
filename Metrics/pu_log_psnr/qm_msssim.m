function [overall_mssim mssim_map] = qm_msssim(img1, img2)
%function overall_mssim = qm_msssim(img1, img2, K, window, level, weight, method)

%QM_MSSSIM Summary of this function goes here
...
if( nargout == 2 )
    mssim_map = ones(size(img1));
    for l = 1:level
        mssim_map = mssim_map .* clamp(imresize(ssim_map_array{l},size(img1)),0,1).^weight(l);
    end
end
