...
for al = 3 : length(algo_list)
    ...
    parfor i = 1 : length(gtNames)
        inp = hdrread([gt_dirs gtNames(i).name]) * 65535; inp = inp(51:670,51:1230,:);
    ...
    mean(Q_score)
    mean(P_score)
    save(['./Result_MATs/HDRv_' algo_name],'mu_psnr','pu2_psnr','pu2_msssim','Q_score','P_score')
end